...
public:
    void recursiveTrace(TreeNode* root) {
        if (root->left != nullptr)
            recursiveTrace(root->left);
...
```
官方题解则使用了循环法的遍历，注意其使用栈进行中序遍历的写法：
```cpp
class Solution {
public:
    int kthSmallest(TreeNode* root, int k) {
...
```
上面两个算法的时间复杂度和空间复杂度都是一样的，均为$O(n)$。
但是，这个算法并没有解决题目的“进阶”中提出的问题，还需要进一步优化。官方题解首先给出了一个思路：
...
根本思路是查看对应子树的结点数。上面这个方法的思路如果只考虑单次运行的情况，那么不会与之前中序遍历的方法拉开差距。但是这个方法的思路可以应用到多次运行的情况，即可以在单次运行后保留一个数组记录本次运行的状态。官方题解给出的实现方式比较长，但核心思路就是记录遇到的二叉树各结点的序号，从而减少此后遍历时的计算量，不必每次都寻找结点的序号。
```cpp
class MyBst {
public:
    MyBst(TreeNode *root) {
...
```
官方题解最后还给出了一种使用平衡二叉搜索树实现的方法。平衡二叉搜索树的最大的特点是，它中序遍历的结果数组具有“平衡”性，能够有效地减少循环时的计算量。但缺点是，上述方法在面试时很难“手撕”，因为需要不断地维护二叉树，使任意的二叉搜索树能够转化成平衡二叉搜索树。
下面的程序不必强求实现，关键在于理解将二叉搜索树变平衡的方法即可（参考[官方题解](https://leetcode-cn.com/problems/balance-a-binary-search-tree/solution/jiang-er-cha-sou-suo-shu-bian-ping-heng-by-leetcod/)）：
```cpp
// 平衡二叉搜索树结点
struct Node {
    int val;
